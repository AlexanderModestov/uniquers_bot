...
Если покрытие частичное: ответь на ту часть, которую поддерживает {context}, и кратко отметь, чего не хватает для полного ответа.

Если есть близкие принципы: дай обобщённый ответ, явно пометив его как «опирается на общие идеи из контекста».
...
При наличии идентификаторов фрагментов укажи их в конце: [источники: S12, S18].

Если релевантность частичная, добавь строку «Чего может не хватать: …» (1 предложение).
