...
async def transcribe_voice_cloud(message: types.Message, supabase_client=None, user_id: int = None) -> str:
                ...
                try:
                    ...
                    if llm_logger:
                        await llm_logger.log_transcription_request(
                            ...
                            success=True,
                            input_metadata={'file_size_bytes': file_size, 'language': 'ru'}
                        )
...
async def handle_user_question(message: types.Message, state: FSMContext, supabase_client):
    ...
    # Get user from database first (needed for logging)
    ...
    if message.text:
        ...
        try:
            user_text = await transcribe_voice_cloud(message, supabase_client, user.id)
